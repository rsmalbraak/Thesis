...
via fine tuning. The models are trained on the CXR training dataset, without augmentation 
by AC-GAN. 
...
train_data_loader = DataLoader(data['train'], batch_size=bs , sampler= train_sampler,num_workers=12)
valid_data_loader = DataLoader(data['train'], batch_size=bs, sampler = valid_sampler,num_workers=12)
test_data_loader = DataLoader(data['test'], batch_size=bs, shuffle=True,num_workers=12)

train_data_loader_resnet = DataLoader(data_resnet['train'], batch_size=bs , sampler= train_sampler,num_workers=12)
valid_data_loader_resnet = DataLoader(data_resnet['train'], batch_size=bs, sampler = valid_sampler,num_workers=12)
test_data_loader_resnet = DataLoader(data_resnet['test'], batch_size=bs, shuffle=True,num_workers=12)

train_data_loader_alexnet = DataLoader(data_alexnet['train'], batch_size=bs , sampler= train_sampler,num_workers=12)
valid_data_loader_alexnet = DataLoader(data_alexnet['train'], batch_size=bs, sampler = valid_sampler,num_workers=12)
...
def train_and_validate_resnet(model, loss_criterion, optimizer, epochs):
    '''
    Function to train and validate ResNet-18
    model : is the model to train and validate
    loss_criterioin : is the loss criterion to minimize
    optimizer : is the optimizer for computing the gradients and updating the weights
    epochs : is the number of epochs
...
# Train the pre-trained SqueezeNet on the un-augmented CXR train dataset
...
# Train the pre-trained ResNet-18 on the un-augmented CXR train dataset
...
# Train the pre-trained AlexNet  on the un-augmented CXR train dataset
