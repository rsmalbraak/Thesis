# -*- coding: utf-8 -*-
"""
Implementation of our Hierarchical AC-GAN (HAC-GAN), which we base on the DCGAN architecture.  
...
and all used modules.  We use the Node2Vec algorithm to generate hierarchy-aware node embeddings.
We use the Node2Vec implementation of Fey, M., & Lenssen, J. E. (2019). Fast graph representation learning with PyTorch Geometric. In
...
output_dir = r'/home/rsmalbraak/output_hierarchical/'
...
##############################################################################
#Set up the Node2Vec algorithm and learn the hierarchy-aware embeddings    
##############################################################################
            
#Make graph
#Ensure that the leaf nodes are node 0, 1, and 2
#These nodes will be used in the embedding layer to retrieve a hierarchy-aware embedding for each class
G = nx.Graph() 
G.add_node(0) #COVID-19 pneumonia
G.add_node(1) #non-COVID-19 pneumonia
G.add_node(2) #normal
G.add_node(3) #all
G.add_node(4) #pneumonia
...
#Call the Node2Vec algorithm, use default parameter settings from original implementation
# Node2Vec implementation from: https://github.com/pyg-team/pytorch_geometric
model = Node2Vec(edge_index, embedding_dim=100, walk_length=80,context_size=10, 
                 walks_per_node=10, num_negative_samples=10, p=1, q=0.5, sparse=True).to(device)
#Define optimizer, adapted from Pytorch Node2Vec implementation: https://github.com/pyg-team/pytorch_geometric/blob/master/examples/node2vec.py
loader = model.loader(batch_size=128, shuffle=True, num_workers=nw)
optimizer = torch.optim.SparseAdam(list(model.parameters()), lr=0.01)

#Train the Node2Vec algorithm
#Training function adapted from https://github.com/pyg-team/pytorch_geometric/blob/master/examples/node2vec.py
def train():
   model.train()
   total_loss = 0
...
#Train Node2Vec for one epoch, as is done in the original implementation
for epoch in range(1, 2):
   loss = train()

#Now use the learned embeddings in the embedding layer of HAC-GAN
embeddings = model()
...
class Generator(nn.Module):
    ...
    def __init__(self, ngpu,nz,ngf,nc,num_class):
        ...
        #Embedding layer now consist of learned embeddings by Node2Vec
    ...
    def forward(self, noise, label):
        ...
        #return the class embedding learned by Node2Vec
        label_embedding = self.embedding[label]
        label_embedding = torch.as_tensor(label_embedding)
...
if __name__ == '__main__':
    ...
    for epoch in range(ne):
        ...
        for i, data in enumerate(trainloader_tf, 0):
            ...
            # Forward fake images into discriminator and calculate "fake" losses
            ...
            #Compute source of D as sum over the real and fake batches
